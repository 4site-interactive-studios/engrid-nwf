 ...
 *
 *  Date: Friday, February 4, 2022 @ 14:46:26 ET
 *  By: bryancasler
 *  ENGrid styles: v0.8.3
 *  ENGrid scripts: v0.8.7
 *
...
__webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = exports.nz = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = __webpack_unused_export__ = void 0;
...
__webpack_unused_export__ = ({ enumerable: true, get: function () { return ste_simple_events_1.SimpleEventDispatcher; } });
...
class loader_Loader {
  constructor() {
    this.logger = new logger_EngridLogger("Logger", "gold", "black", "🔁");
  ...
  reload() {
    ...
    switch (assets) {
      case "local":
        this.logger.log("ENgrid Loader: LOADING LOCAL"); // Find a way to guess local URL if there's no engrid_repo
...
class App extends engrid_ENGrid {
  ...
  run() {
    ...
    new live_variables_LiveVariables(this.options); // Dynamically set Recurrency Frequency

    new set_recurr_freq_setRecurrFreq(); // Upsell Lightbox

    new upsell_lightbox_UpsellLightbox(); // Amount Labels

    new amount_label_AmountLabel(); // On the end of the script, after all subscribers defined, let's load the current value
    ...
    if (this.options.TranslateFields) new translate_fields_TranslateFields(); // Simple Country Select

    new simple_country_select_SimpleCountrySelect(); // Add Image Attribution

    if (this.options.MediaAttribution) new media_attribution_MediaAttribution(); // Apple Pay

    if (this.options.applePay) new apple_pay_ApplePay(); // Capitalize Fields

    if (this.options.CapitalizeFields) new capitalize_fields_CapitalizeFields(); // Auto Year Class

    if (this.options.AutoYear) new auto_year_AutoYear(); // Credit Card Numbers Only

    new credit_card_numbers_CreditCardNumbers(); // Autocomplete Class

    new autocomplete_Autocomplete(); // Ecard Class

    new ecard_Ecard(); // Click To Expand
    ...
    if (this.options.RememberMe && typeof this.options.RememberMe === "object") new remember_me_RememberMe(this.options.RememberMe);
  ...
  onSubmit() {
    ...
    if (this.inIframe()) {
      iframe_sendIframeFormStatus("submit");
    }
  }
...
class credit_card_numbers_CreditCardNumbers {
  constructor() {
    this._form = en_form_EnForm.getInstance();
...
const iframe_sendIframeFormStatus = status => {
  window.parent.postMessage({
    status: status,
    pageNumber: engrid_ENGrid.getPageNumber(),
    pageCount: engrid_ENGrid.getPageCount(),
...
class simple_country_select_SimpleCountrySelect {
  constructor() {
...
const crumbs = {
  ...
  ls: {
    ...
    get: function (key) {
      let asJSON = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    ...
    getAll: function () {
      let asJSON = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
...
const interfaces_options_OptionsDefaults = {
    backgroundImage: "",
    MediaAttribution: true,
    applePay: false,
    CapitalizeFields: false,
...
const interfaces_upsell_options_UpsellOptionsDefaults = {
    image: "https://picsum.photos/480/650",
    imagePosition: "left",
    title: "Will you change your gift to just {new-amount} a month to boost your impact?",
    paragraph: "Make a monthly pledge today to support us with consistent, reliable resources during emergency moments.",
...
const interfaces_translate_options_TranslateOptionsDefaults = {
...
var strongly_typed_events_dist = __webpack_require__(5363);
...
class events_en_form_EnForm {
    constructor() {
        ...
        this._onSubmit = new strongly_typed_events_dist/* SignalDispatcher */.nz();
        this._onValidate = new strongly_typed_events_dist/* SignalDispatcher */.nz();
        this._onError = new strongly_typed_events_dist/* SignalDispatcher */.nz();
        this.submit = true;
        this.validate = true;
    }
    static getInstance() {
...
class events_donation_amount_DonationAmount {
    ...
    static getInstance(radios = "transaction.donationAmt", other = "transaction.donationAmt.other") {
        if (!events_donation_amount_DonationAmount.instance) {
            events_donation_amount_DonationAmount.instance = new events_donation_amount_DonationAmount(radios, other);
        }
        return events_donation_amount_DonationAmount.instance;
    }
    get amount() {
        return this._amount;
    }
...
class events_donation_frequency_DonationFrequency {
    constructor() {
    ...
    static getInstance() {
        if (!events_donation_frequency_DonationFrequency.instance) {
            events_donation_frequency_DonationFrequency.instance = new events_donation_frequency_DonationFrequency();
        }
        return events_donation_frequency_DonationFrequency.instance;
    }
    get frequency() {
        return this._frequency;
    }
...
class events_processing_fees_ProcessingFees {
    constructor() {
    ...
    static getInstance() {
        if (!events_processing_fees_ProcessingFees.instance) {
            events_processing_fees_ProcessingFees.instance = new events_processing_fees_ProcessingFees();
        }
        return events_processing_fees_ProcessingFees.instance;
    }
    get onFeeChange() {
        return this._onFeeChange.asEvent();
    }
...
class app_App extends (/* unused pure expression or super */ null && (ENGrid)) {
    constructor(options) {
        super();
        // Events
        this._form = EnForm.getInstance();
...
class dist_amount_label_AmountLabel {
    constructor() {
        this._frequency = DonationFrequency.getInstance();
        if (!this.shouldRun()) {
            // If we're not on a Donation Page, get out
...
class dist_engrid_ENGrid {
    ...
    static createHiddenInput(name, value = "") {
        ...
        input.classList.add("engrid-added-input");
        input.value = value;
        dist_engrid_ENGrid.enForm.appendChild(input);
        return input;
    }
    // Trigger EN Dependencies
    static enParseDependencies() {
        ...
        if (window.EngagingNetworks &&
            ...
            window.EngagingNetworks.require._defined.enDependencies.dependencies.parseDependencies(window.EngagingNetworks.dependencies);
            if (dist_engrid_ENGrid.getOption("Debug"))
    ...
    performValidation(url) {
        return new Promise(function (resolve, reject) {
            ...
            var validationData = "&merchantIdentifier=" +
                apple_pay_merchantIdentifier +
                "&merchantDomain=" +
                apple_pay_merchantDomainName +
                "&displayName=" +
                apple_pay_merchantDisplayName;
            var validationUrl = "/ea-dataservice/rest/applepay/validateurl?url=" + url + validationData;
            var xhr = new XMLHttpRequest();
            xhr.onload = function () {
                var data = JSON.parse(this.responseText);
...
class dist_capitalize_fields_CapitalizeFields {
    constructor() {
        this._form = EnForm.getInstance();
        this._form.onSubmit.subscribe(() => this.capitalizeFields("en__field_supporter_firstName", "en__field_supporter_lastName", "en__field_supporter_address1", "en__field_supporter_city"));
    }
...
class dist_credit_card_numbers_CreditCardNumbers {
    constructor() {
        this._form = EnForm.getInstance();
        this.ccField = document.getElementById("en__field_transaction_ccnumber");
        if (this.ccField) {
...
class dist_autocomplete_Autocomplete {
    constructor() {
...
// Works when the user has adds ".click-to-expand" as a class to any field
class dist_click_to_expand_ClickToExpand {
    constructor() {
        this.clickToExpandWrapper = document.querySelectorAll("div.click-to-expand");
        if (this.clickToExpandWrapper.length) {
            this.clickToExpandWrapper.forEach((element) => {
...
const custom_methods_body = document.body;
const custom_methods_enGrid = document.getElementById("engrid");
const custom_methods_enInput = (() => {
    /************************************
     * Globablly Scoped Constants and Variables
     ***********************************/
    // @TODO Needs to be expanded to bind other EN elements (checkbox, radio) and compound elements (split-text, split-select, select with other input, etc...)
...
const custom_methods_bindEvents = (e) => {
    /* @TODO */
    /************************************
     * INPUT, TEXTAREA, AND SELECT ACTIVITY CLASSES (FOCUS AND BLUR)
     * NOTE: STILL NEEDS WORK TO FUNCTION ON "SPLIT" CUSTOM EN FIELDS
...
const custom_methods_removeClassesByPrefix = (el, prefix) => {
    for (var i = el.classList.length - 1; i >= 0; i--) {
        if (el.classList[i].startsWith(prefix)) {
            el.classList.remove(el.classList[i]);
        }
...
const custom_methods_debugBar = () => {
    if (window.location.href.indexOf("debug") != -1 ||
        location.hostname === "localhost" ||
        location.hostname === "127.0.0.1") {
        custom_methods_body.classList.add("debug");
        if (custom_methods_enGrid) {
            custom_methods_enGrid.insertAdjacentHTML("beforebegin", '<span id="debug-bar">' +
        ...
        const fancyErrorsToggle = () => {
            if (custom_methods_enGrid) {
                custom_methods_enGrid.classList.toggle("fancy-errors");
            }
        };
        const pageEdit = () => {
            window.location.href = window.location.href + "?edit";
        ...
        const layoutToggle = () => {
            if (custom_methods_enGrid) {
                if (custom_methods_enGrid.classList.contains("layout-centercenter1col")) {
                    custom_methods_removeClassesByPrefix(custom_methods_enGrid, "layout-");
                    custom_methods_enGrid.classList.add("layout-centerright1col");
                }
                else if (custom_methods_enGrid.classList.contains("layout-centerright1col")) {
                    custom_methods_removeClassesByPrefix(custom_methods_enGrid, "layout-");
                    custom_methods_enGrid.classList.add("layout-centerleft1col");
                }
                else if (custom_methods_enGrid.classList.contains("layout-centerleft1col")) {
                    custom_methods_removeClassesByPrefix(custom_methods_enGrid, "layout-");
                    custom_methods_enGrid.classList.add("layout-embedded");
                }
                else if (custom_methods_enGrid.classList.contains("layout-embedded")) {
                    custom_methods_removeClassesByPrefix(custom_methods_enGrid, "layout-");
                    custom_methods_enGrid.classList.add("layout-centercenter1col");
                }
                else {
                    console.log("While trying to switch layouts, something unexpected happen.");
                }
        ...
        const debugClose = () => {
            custom_methods_body.classList.remove("debug");
            const debugBar = document.getElementById("debug-bar");
            if (debugBar) {
                debugBar.style.display = "none";
            }
...
const custom_methods_inputPlaceholder = () => {
    // FIND ALL COMMON INPUT FIELDS
    let enFieldDonationAmt = document.querySelector(".en__field--donationAmt.en__field--withOther .en__field__input--other");
    let enFieldFirstName = document.querySelector("input#en__field_supporter_firstName");
    let enFieldLastName = document.querySelector("input#en__field_supporter_lastName");
...
const custom_methods_preventAutocomplete = () => {
    let enFieldDonationAmt = document.querySelector(".en__field--donationAmt.en__field--withOther .en__field__input--other");
    if (enFieldDonationAmt) {
        enFieldDonationAmt.setAttribute("autocomplete", "off");
    }
...
const custom_methods_watchInmemField = () => {
    const enFieldTransactionInmem = document.getElementById("en__field_transaction_inmem");
    const handleEnFieldTransactionInmemChange = (e) => {
        if (custom_methods_enGrid) {
            if (enFieldTransactionInmem.checked) {
                custom_methods_enGrid.classList.add("has-give-in-honor");
            }
            else {
                custom_methods_enGrid.classList.remove("has-give-in-honor");
            }
        }
    };
    // Check Give In Honor State on Page Load
    if (enFieldTransactionInmem && custom_methods_enGrid) {
        // Run on page load
        if (enFieldTransactionInmem.checked) {
            custom_methods_enGrid.classList.add("has-give-in-honor");
        }
...
// @TODO Refactor (low priority)
const custom_methods_watchGiveBySelectField = () => {
    const enFieldGiveBySelect = document.querySelector(".en__field--give-by-select");
    const transactionGiveBySelect = document.getElementsByName("transaction.giveBySelect");
    const enFieldPaymentType = document.querySelector("#en__field_transaction_paymenttype");
    let enFieldGiveBySelectCurrentValue = document.querySelector('input[name="transaction.giveBySelect"]:checked');
    ...
    const handleEnFieldGiveBySelect = (e) => {
        ...
        if (enFieldGiveBySelectCurrentValue &&
            ...
            if (custom_methods_enGrid) {
                custom_methods_removeClassesByPrefix(custom_methods_enGrid, prefix);
                custom_methods_enGrid.classList.add("has-give-by-card");
            }
            // enFieldPaymentType.value = "card";
            custom_methods_handleCCUpdate();
        }
        else if (enFieldGiveBySelectCurrentValue &&
            enFieldGiveBySelectCurrentValue.value.toLowerCase() == "ach") {
            if (custom_methods_enGrid) {
                custom_methods_removeClassesByPrefix(custom_methods_enGrid, prefix);
                custom_methods_enGrid.classList.add("has-give-by-ach");
            }
            enFieldPaymentType.value = "ACH";
        }
        else if (enFieldGiveBySelectCurrentValue &&
            ...
            if (custom_methods_enGrid) {
                custom_methods_removeClassesByPrefix(custom_methods_enGrid, prefix);
                custom_methods_enGrid.classList.add("has-give-by-paypal");
            }
            enFieldPaymentType.value = "paypal";
        }
        else if (enFieldGiveBySelectCurrentValue &&
            ...
            if (custom_methods_enGrid) {
                custom_methods_removeClassesByPrefix(custom_methods_enGrid, prefix);
                custom_methods_enGrid.classList.add("has-give-by-applepay");
            }
            enFieldPaymentType.value = "applepay";
        }
    };
    ...
    if (enFieldGiveBySelect) {
        ...
        if (enFieldGiveBySelectCurrentValue &&
            ...
            if (custom_methods_enGrid) {
                custom_methods_removeClassesByPrefix(custom_methods_enGrid, prefix);
                custom_methods_enGrid.classList.add("has-give-by-check");
            }
            enFieldPaymentType.value = "ACH";
        }
        else if (enFieldGiveBySelectCurrentValue &&
 ...
 * Input fields as reference variables
 */
const custom_methods_field_credit_card = document.getElementById("en__field_transaction_ccnumber");
const custom_methods_field_payment_type = document.getElementById("en__field_transaction_paymenttype");
let custom_methods_field_expiration_parts = document.querySelectorAll(".en__field--ccexpire .en__field__input--splitselect");
const custom_methods_field_country = document.getElementById("en__field_supporter_country");
let custom_methods_field_expiration_month = custom_methods_field_expiration_parts[0];
let custom_methods_field_expiration_year = custom_methods_field_expiration_parts[1];
/* The Donation Other Giving Amount is a "Number" type input field.
   It also has its step value set to .01 so it increments up/down by once whole cent.
   This step also client-side prevents users from entering a fraction of a penny.
   And it has a min set to 5 so nothing less can be submitted
*/
const custom_methods_SetEnFieldOtherAmountRadioStepValue = () => {
    const enFieldOtherAmountRadio = document.querySelector(".en__field--donationAmt .en__field__input--other");
    if (enFieldOtherAmountRadio) {
        enFieldOtherAmountRadio.setAttribute("step", ".01");
        enFieldOtherAmountRadio.setAttribute("type", "number");
 ...
 */
// current_month and current_year used by handleExpUpdate()
let custom_methods_d = new Date();
var custom_methods_current_month = custom_methods_d.getMonth() + 1; // month options in expiration dropdown are indexed from 1
var custom_methods_current_year = custom_methods_d.getFullYear() - 2000;
// getCardType used by handleCCUpdate()
const custom_methods_getCardType = (cc_partial) => {
    let key_character = cc_partial.charAt(0);
    const prefix = "live-card-type-";
    const field_credit_card_classes = custom_methods_field_credit_card.className
        .split(" ")
        .filter((c) => !c.startsWith(prefix));
    switch (key_character) {
        case "0":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-invalid");
            return false;
        case "1":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-invalid");
        ...
        case "3":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-amex");
            return "amex";
        case "4":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-visa");
            return "visa";
        case "5":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-mastercard");
            return "mastercard";
        case "6":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-discover");
            return "discover";
        case "7":
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-invalid");
        ...
        default:
            custom_methods_field_credit_card.className = field_credit_card_classes.join(" ").trim();
            custom_methods_field_credit_card.classList.add("live-card-type-na");
            return false;
    }
};
/*
 * Handlers
 */
const custom_methods_handleCCUpdate = () => {
    const card_type = custom_methods_getCardType(custom_methods_field_credit_card.value);
    const card_values = {
        amex: ["amex", "american express", "americanexpress", "amx", "ax"],
        visa: ["visa", "vi"],
        mastercard: ["mastercard", "master card", "mc"],
    ...
    const selected_card_value = card_type
        ? Array.from(custom_methods_field_payment_type.options).filter((d) => card_values[card_type].includes(d.value.toLowerCase()))[0].value
        : "";
    if (custom_methods_field_payment_type.value != selected_card_value) {
        custom_methods_field_payment_type.value = selected_card_value;
        const paymentTypeChangeEvent = new Event("change", { bubbles: true });
        custom_methods_field_payment_type.dispatchEvent(paymentTypeChangeEvent);
    }
};
const custom_methods_handleExpUpdate = (e) => {
    // handle if year is changed to current year (disable all months less than current month)
    // handle if month is changed to less than current month (disable current year)
    if (e == "month") {
        let selected_month = parseInt(custom_methods_field_expiration_month.value);
        let disable = selected_month < custom_methods_current_month;
        console.log("month disable", disable, typeof disable, selected_month, custom_methods_current_month);
        for (let i = 0; i < custom_methods_field_expiration_year.options.length; i++) {
            // disable or enable current year
            if (parseInt(custom_methods_field_expiration_year.options[i].value) <= custom_methods_current_year) {
                if (disable) {
                    //@TODO Couldn't get working in TypeScript
                    custom_methods_field_expiration_year.options[i].setAttribute("disabled", "disabled");
                }
                else {
                    custom_methods_field_expiration_year.options[i].disabled = false;
                }
            }
        }
    }
    else if (e == "year") {
        let selected_year = parseInt(custom_methods_field_expiration_year.value);
        let disable = selected_year == custom_methods_current_year;
        console.log("year disable", disable, typeof disable, selected_year, custom_methods_current_year);
        for (let i = 0; i < custom_methods_field_expiration_month.options.length; i++) {
            // disable or enable all months less than current month
            if (parseInt(custom_methods_field_expiration_month.options[i].value) < custom_methods_current_month) {
                if (disable) {
                    //@TODO Couldn't get working in TypeScript
                    custom_methods_field_expiration_month.options[i].setAttribute("disabled", "disabled");
                }
                else {
                    custom_methods_field_expiration_month.options[i].disabled = false;
                }
            }
        }
    }
 ...
 * Event Listeners
 */
if (custom_methods_field_credit_card) {
    custom_methods_field_credit_card.addEventListener("keyup", function () {
        custom_methods_handleCCUpdate();
    });
    custom_methods_field_credit_card.addEventListener("paste", function () {
        custom_methods_handleCCUpdate();
    });
    custom_methods_field_credit_card.addEventListener("blur", function () {
        custom_methods_handleCCUpdate();
    });
}
if (custom_methods_field_expiration_month && custom_methods_field_expiration_year) {
    custom_methods_field_expiration_month.addEventListener("change", function () {
        custom_methods_handleExpUpdate("month");
    });
    custom_methods_field_expiration_year.addEventListener("change", function () {
        custom_methods_handleExpUpdate("year");
    });
}
// EN Polyfill to support "label" clicking on Advocacy Recipient "labels"
const custom_methods_contactDetailLabels = () => {
...
// values from your account. This means to unsubscribe the user has to uncheck the
// newsletter checkbox(s) before submitting.
const custom_methods_simpleUnsubscribe = () => {
    // console.log("simpleUnsubscribe fired");
    // Check if we're on an Unsubscribe / Manage Subscriptions page
    if (window.location.href.indexOf("/subscriptions") != -1) {
        // console.log("On an subscription management page");
...
// Watch the Region Field for changes. If there is only one option, hide it.
// @TODO Should this be expanded where if a select only has one option it's always hidden?
const custom_methods_country_select = document.getElementById("en__field_supporter_country");
const custom_methods_region_select = document.getElementById("en__field_supporter_region");
if (custom_methods_country_select) {
    custom_methods_country_select.addEventListener("change", () => {
        setTimeout(() => {
            if (custom_methods_region_select.options.length == 1 &&
                custom_methods_region_select.options[0].value == "other") {
                custom_methods_region_select.classList.add("hide");
            }
            else {
                custom_methods_region_select.classList.remove("hide");
            }
        }, 100);
    });
}
// @TODO "Footer in Viewport Check" should be made its own TS file
const custom_methods_contentFooter = document.querySelector(".content-footer");
const custom_methods_isInViewport = (e) => {
...
// Checks to see if the page is so short, the footer is above the fold. If the footer is above the folde we'll use this class to ensure at a minimum the page fills the full viewport height.
if (custom_methods_contentFooter && custom_methods_isInViewport(custom_methods_contentFooter)) {
    document
...
const dist_iframe_sendIframeHeight = () => {
    let height = document.body.offsetHeight;
    console.log("Sending iFrame height of: ", height, "px"); // check the message is being sent correctly
    window.parent.postMessage({
        frameHeight: height,
...
const dist_iframe_sendIframeFormStatus = (status) => {
    window.parent.postMessage({
        status: status,
        pageNumber: ENGrid.getPageNumber(),
        pageCount: ENGrid.getPageCount(),
...
class dist_media_attribution_MediaAttribution {
    constructor() {
        // Find all images with attribution but not with the "data-attribution-hide-overlay" attribute
        this.mediaWithAttribution = document.querySelectorAll("img[data-attribution-source]:not([data-attribution-hide-overlay]), video[data-attribution-source]:not([data-attribution-hide-overlay])");
        this.mediaWithAttribution.forEach((element) => {
...
class dist_live_variables_LiveVariables {
    constructor(options) {
        var _a;
        this._amount = DonationAmount.getInstance();
        this._fees = ProcessingFees.getInstance();
...
class dist_upsell_lightbox_UpsellLightbox {
    constructor() {
        this.overlay = document.createElement("div");
        this._form = EnForm.getInstance();
        this._amount = DonationAmount.getInstance();
...
// This class works when the user has added ".simple_country_select" as a class in page builder for the Country select
class dist_translate_fields_TranslateFields {
    constructor() {
        this.countrySelect = document.querySelector("#en__field_supporter_country");
        this.stateField = document.querySelector("#en__field_supporter_region");
        let options = "EngridTranslate" in window ? window.EngridTranslate : {};
...
class dist_simple_country_select_SimpleCountrySelect {
    constructor() {
        this.countryWrapper = document.querySelector(".simple_country_select");
        this.countrySelect = document.querySelector("#en__field_supporter_country");
        this.country = null;
...
class dist_skip_link_SkipToMainContentLink {
    constructor() {
        const firstTitleInEngridBody = document.querySelector("div[class*='body-'] title");
        const firstH1InEngridBody = document.querySelector("div[class*='body-'] h1");
        const firstTitle = document.querySelector("title");
...
class dist_set_recurr_freq_setRecurrFreq {
    constructor() {
        this._frequency = DonationFrequency.getInstance();
        this.linkClass = "setRecurrFreq-";
...
class dist_neverbounce_NeverBounce {
...
class dist_progress_bar_ProgressBar {
    constructor() {
        var _a, _b;
        const progressIndicator = document.querySelector("span[data-engrid-progress-indicator]");
...
class dist_remember_me_RememberMe {
    constructor(options) {
        this._form = EnForm.getInstance();
        this.iframe = null;
        this.remoteUrl = (options.remoteUrl) ? options.remoteUrl : null;
...
class dist_show_if_amount_ShowIfAmount {
    constructor() {
        this._amount = DonationAmount.getInstance();
...
class dist_other_amount_OtherAmount {
    constructor() {
...
class XVerify {
  constructor(options) {
    _defineProperty(this, "form", events_en_form_EnForm.getInstance());
    ...
    this.xvDate = this.options.dateField ? document.querySelector(`[name="${this.options.dateField}"]`) : null;
    ...
    if (this.options.statusField && !this.xvStatus) {
      this.xvStatus = dist_engrid_ENGrid.createHiddenInput(this.options.statusField);
    }

    if (this.options.dateField && !this.xvDate) {
      this.xvDate = dist_engrid_ENGrid.createHiddenInput(this.options.dateField);
    }

    this.init();
    if (dist_engrid_ENGrid.debug) console.log("Engrid Xverify: LOADED", this.emailField);
  }
  ...
  init() {
    if (!this.emailField) {
      if (dist_engrid_ENGrid.debug) console.log("Engrid Xverify: E-mail Field Not Found", this.emailField);
      return;
    }
  ...
  validateEmail(email) {
    if (!this.emailField) {
      if (dist_engrid_ENGrid.debug) console.log("Engrid XVerify validate(): E-mail Field Not Found. Returning true.");
      return true;
    }

    dist_engrid_ENGrid.disableSubmit("Validating Your Email");
    dist_engrid_ENGrid.loadJS(this.apiURL + "&email=" + email, () => {
  ...
  static validateXverify(data) {
    ...
    if (dist_engrid_ENGrid.debug) console.log("Engrid XVerify validateXverify():", data);
    ...
    if (xv.xvDate) {
      xv.xvDate.value = xv.xvDate.value = dist_engrid_ENGrid.formatDate(new Date(), xv.xvDateFormat);
    }

    xv.emailField.dataset.xverifyStatus = data.email.status;
    xv.emailField.dataset.xverifyDate = dist_engrid_ENGrid.formatDate(new Date(), xv.xvDateFormat);
    ...
    if (!["accept_all", "unknown", "valid", "risky"].includes(data.email.status)) {
      ...
      if (dist_engrid_ENGrid.debug) console.log("Engrid XVerify validateXverify():", "INVALID");
      xv.invalid(data.email.message);
      return false;
    }

    if (dist_engrid_ENGrid.debug) console.log("Engrid XVerify validateXverify():", "VALID");
    xv.valid();
    return true;
  }
